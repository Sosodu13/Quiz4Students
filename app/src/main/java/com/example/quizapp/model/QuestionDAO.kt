...
interface QuestionDAO {
    ...
    @Query("SELECT * FROM question \n" +
            ...
            "THEN 3 \n" +
            "WHEN concept.tag = 'Maitrise' \n" +
            "THEN 4 END ASC, random() \n" +
            "LIMIT 10")
    fun getWithoutFilter() : List<Question>


    @Query("SELECT * FROM cours co LEFT JOIN themecours tc ON co.id = tc.cours LEFT JOIN theme t ON tc.theme = t.id LEFT JOIN concepttheme ct ON t.id = ct.theme LEFT JOIN concept c ON ct.concept = c.id LEFT JOIN question q ON c.id = q.concept_id WHERE co.id=:param ORDER BY CASE WHEN c.tag = 'Non commencé' THEN 1 WHEN c.tag = 'Initiation' THEN 2 WHEN c.tag = 'Compréhension' THEN 3 WHEN c.tag = 'Maitrise' THEN 4 END ASC, random() LIMIT 10")
    ...
    @Query("SELECT * FROM question left join concept on question.concept_id = concept.id WHERE concept_id in (:param) ORDER BY CASE WHEN concept.tag = 'Non commencé' THEN 1 WHEN concept.tag = 'Initiation' THEN 2 WHEN concept.tag = 'Compréhension' THEN 3 WHEN concept.tag = 'Maitrise' THEN 4 END ASC, random() LIMIT 10")
